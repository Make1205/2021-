...
# 读入数据
google_stock = pd.read_excel('1 (10).xlsx')
...
# 划分训练集与验证集
google_stock = google_stock[['volume']]
lt=19155
...
scaler.fit_transform(pd.DataFrame(valid['volume'].values))
...
plt.xlabel('ticks')
plt.ylabel('volumes')
plt.title('volumes in future 30 ticks')
...
dataframe=pd.DataFrame(data_pd[0:4514])
dataframe.to_excel('y10v1.xlsx')
plt.savefig('E:\pic\pic10v', dpi=320)